...
class Unique_onlinespecial_model extends CI_Model{
	
	var $table = 'tbl_unique_specialoffer';
	...
	function addOffer(){
		...
		if($this->query_model->insertData($this->table,$data)):
			...
			if($data['upsale'] == 1){
					...
					$this->query_model->insertData('tbl_unique_onlinespecial_upsales',$updata);
				}
			}
		
			redirect("admin/unique_onlinespecial/view/".$data['cat_id']);
		endif;
	}
	...
	function updateOffer(){
		...
		if($data['upsale'] == 1){
			if(!empty($_POST['data'])){
			$this->query_model->deletebySpecific('tbl_unique_onlinespecial_upsales','trial_offer_id',$this->uri->segment(4));
				foreach($_POST['data'] as $upsalePage){
