	 ...
	 public function userMedia($user_id, $token){
		...
		return $json = json_decode($content, true);*/
			...
			if(!empty($response)){
				...
				//echo '<prE>json'; print_r($json); die;
			}
...
public function checkActiveCampaignContactExists($activeCampaign, $email){
	...
	public function replaceAutoResponderVaribles($content, $formData, $extraContentArr){
		...
		$view_message_link = (isset($formData['twilio_msg_user_id']) && !empty($formData['twilio_msg_user_id'])) ? 'To reply click : '.base_url().'admin/sms/chat/'.strtr(base64_encode($formData['twilio_msg_user_id']), '+/', '-_') : '';
		...
		if(!empty($twilioChatApi) && $twilioChatApi[0]->type == 1){
			...
			if(!empty($twilioChatApi[0]->sid) && !empty($twilioChatApi[0]->token) ){
				...
				if(!empty($msgData)){
					...
					$twilio_user_name = (isset($msgData['twilio_user_name']) && !empty($msgData['twilio_user_name'])) ? $msgData['twilio_user_name'] : '';
					$phone = (isset($msgData['phone']) && !empty($msgData['phone'])) ? $msgData['phone'] : '';
					$author_name = (isset($msgData['author_name']) && !empty($msgData['author_name'])) ? $msgData['author_name'] : '';
					
					$reciever_by = (isset($msgData['reciever_by']) && !empty($msgData['reciever_by'])) ? $msgData['reciever_by'] : '';
					...
					if(!empty($phone) && !empty($message) && !empty($twilio_user_id)){
						...
						/*$msg_record_student_chatboard = $twilio->conversations->v1->conversations('CHbe6073113e71457aaf2745b310a028ce')
                                     ->messages("IM3aed9fe739154b6f860dce64df74a6bc")
                                     ->fetch();
						echo '<pre>msg_record_student_chatboard'; print_r($msg_record_student_chatboard); die;*/
						
						$clientPhoneNumber = trim(str_replace(array("(",")"," ","-"),'',$phone));
						if($reciever_by == "admin"){
							$clientPhoneNumber = $clientPhoneNumber;
							//echo 'admin==>'. $clientPhoneNumber; die;
						}else{
						...
						try{
						  ...
						  if(!empty($user_detail) && !empty($user_detail[0]->chat_conversation_sid) ){
							...
							/*if($reciever_by == "admin"){
								echo '<pre>clientPhoneNumber'; print_r($clientPhoneNumber);
								echo '<pre>user_detail'; print_r($user_detail);
								echo '<pre>message'; print_r($message); die;
							}	*/
							
							$updateData = array();
							$updateData['name']  =  $twilio_user_name;
							...
							//echo 'message old id=>'.$message->sid.'<br/>';
							 ...
							 $service = $twilio->messaging->v1->services
								...
								if(isset($service->sid) && !empty($service->sid) ){
									...
									//echo 'service id=>'.$service->sid.'<br/>'; 
	...
	public function saveMsgFromTwilioChatApi($data, $leadData = array()){
		...
		if(!empty($twilioChatApi) && $twilioChatApi[0]->type == 1){
			...
			if(!empty($formData)){
				...
				$page_url = $this->query_model->reconstruct_url($page_url);
				$page_url = $this->query_model->getFormPageUrl($page_url,$_POST);
				...
				if(!empty($twilioUserArr['name']) && !empty($twilioUserArr['phone'])){
					...
					$twilio_sms_template = ($twilioUserArr['lead_type'] == "trial_forms") ? 'paid_trial_template' : 'opt_in_form_template';
					...
					if(!empty($twilio_user_id)){
						...
						if($twilio_sms_template == "paid_trial_template"){
							...
							$this->db->where('template_type',$twilio_sms_template);
							$twilio_sms_flow_template = $this->query_model->getBySpecific('tbl_twilio_sms_templates','sms_flow_id',$twilio_sms_flows[0]->id);
						...
						if(!empty($twilio_sms_flows) && !empty($twilio_sms_flow_template)){
							...
							/*$template_start_time = $twilio_sms_flow_template[0]->start_time;
							$template_end_time = $twilio_sms_flow_template[0]->end_time;
							...
							//echo '<prE>formData'; print_r($formData); die;
							$msg_template = $this->query_model->replaceAutoResponderVaribles($twilio_sms_flow_template[0]->msg_template, $formData, '');
						...
						/******** SEND ADMIN SMS ****/
							$admin_phone_number = trim(str_replace(array("(",")"," ","-"),'',$twilioChatApi[0]->twilio_admin_phone_number));
							$this->db->where('phone',$admin_phone_number);
							$adminUserRecord = $this->query_model->getBySpecific('twilio_sms_users','conversation_type','admin');
							//echo '<pre>adminUserRecord'; print_r($adminUserRecord); die;
							if(empty($adminUserRecord)){
								...
								$this->query_model->insertData('twilio_sms_users',$twilioAdminUserArr);
								$twilio_admin_user_id = $this->db->insert_id();
								$admin_user_type = "add";
								
								//$admin_phone_number = trim(str_replace(array("(",")"," ","-"),'',$twilioChatApi[0]->twilio_admin_phone_number));
								$this->db->select(array('id','name','phone'));
								$this->db->where('conversation_type','admin');
								$adminUserRecord = $this->query_model->getBySpecific('twilio_sms_users','phone',$admin_phone_number);
							}else{
									
								$twilio_admin_user_id = $adminUserRecord[0]->id;
								$admin_user_type = "edit"; 
								
							}
							
							$admin_twilio_sms_template = 'admin_sms_template';
							
							$admin_twilio_sms_flows = $this->query_model->getBySpecific('twilio_sms_flows','page_url','all_forms');
							$this->db->where('template_type',$admin_twilio_sms_template);
							$admin_twilio_sms_flow_template = $this->query_model->getBySpecific('tbl_twilio_sms_templates','sms_flow_id',$admin_twilio_sms_flows[0]->id);
							
							if(!empty($admin_twilio_sms_flows) && !empty($admin_twilio_sms_flow_template)){
								
								$formData['twilio_msg_user_id'] = $twilio_user_id;
								$admin_msg_template = $this->query_model->replaceAutoResponderVaribles($admin_twilio_sms_flow_template[0]->msg_template, $formData, '');
								
								unset($formData['twilio_msg_user_id']);
								$admin_msg_template_cms = $this->query_model->replaceAutoResponderVaribles($admin_twilio_sms_flow_template[0]->msg_template, $formData, '');
								
								
								//echo '<pre>admin_msg_template_cms'; print_r($admin_msg_template_cms); die;
								
								$insertSMSData = array();
								$insertSMSData['sender_by'] = 'student';
								$insertSMSData['admin_id'] = $twilio_admin_user_id;
								$insertSMSData['sms_users_id'] = $twilio_user_id;
								$insertSMSData['message'] = $admin_msg_template_cms;
								...
								//$admin_phone = (!empty($twilio_user_detail) && !empty($twilio_user_detail[0]->phone)) ? $twilio_user_detail[0]->phone : '';
									...
									if(!empty($messages)){
										...
										foreach ($messages as $msg) {
											...
											if(empty($msg_exit)){
												...
												$twilio_user_id = $twilio_user->id;
												$twilio_user_name = $twilio_user->name;
												
												$insertSMSData = array();
												
												if($twilio_user->conversation_type == "admin"){
													if(!empty($msg->author)){
														$msg_user_id = explode('_',$msg->author);
														$msg_user_id = (isset($msg_user_id[1]) && !empty($msg_user_id[1])) ? $msg_user_id[1] : '';
														
														if(!empty($msg_user_id)){
															
															$this->db->select(array('id','name','phone','chat_conversation_sid','conversation_type'));
															$this->db->where('id',$msg_user_id);
															$user_detail = $this->query_model->getBySpecific('twilio_sms_users','is_deleted',0);
															
															if(!empty($user_detail)){
																$insertSMSData['sender_by'] = 'admin';
																$insertSMSData['admin_id'] = $twilio_user_id;
																$insertSMSData['sms_users_id'] = $user_detail[0]->id;
																$phone = $user_detail[0]->phone;
																$author_name = 'Dojo Admin';
																$reciever_by = 'student';
																$msg_type = 'admin_to_student';
																$twilio_user_name = $user_detail[0]->name;
																$twilio_user_id = $user_detail[0]->id;
															}
													...
													if($msg->author == "Dojo Admin"){
														...
														$phone = (!empty($twilio_user) && !empty($twilio_user->phone)) ? $twilio_user->phone : '';
														$author_name = 'user_'.$twilio_user_id;
														$reciever_by = 'student';
														$msg_type = 'admin_to_student';
													}else{
														$insertSMSData['sender_by'] = 'student';
														...
														$phone = $twilioChatApi[0]->twilio_admin_phone_number;
														$author_name = 'Dojo Admin';
														$reciever_by = 'admin';
												...
												if($msg->author != "Dojo Admin"){
													...
													$message = $twilio_user->name.' sent you a message :- '. $msg->body.'. To reply click :'.base_url().'admin/sms/chat/'.strtr(base64_encode($twilio_user->id), '+/', '-_');
												}
												...
												//echo 'twilio_sms_msg_id==><'.$twilio_sms_msg_id; die;
												...
												$chat_conversation_sid = $twilio_user->chat_conversation_sid;
												$chat_message_sid = $msg->sid;
												$delete_msg = $twilio->conversations->v1->conversations($chat_conversation_sid)
																	  ->messages($chat_message_sid)
																		  ->delete();
												
												// send again new msg admin or student ///
												$this->query_model->sendMsgTwilioChatApi($msgData);
	...
	if(!empty($order_id) && !empty($table_name)){
		...
		$leadTypes = array('tbl_dojocart_orders'=>'dojocart_lead','tblbirthdayparty'=>'birthday_party_lead','tblorders'=>'trial_offer_lead');
			...
			if(!empty($twilioChatApi) && $twilioChatApi[0]->type == 1){
				...
				if(!empty($twilioChatApi[0]->sid) && !empty($twilioChatApi[0]->token) ){
					$lead_type = isset($leadTypes[$table_name]) ? $leadTypes[$table_name] : '';
					if($lead_type == "trial_offer_lead" || $lead_type == "birthday_party_lead"){
						$insertData = array();
						$insertData['lead_type'] = isset($leadTypes[$table_name]) ? $leadTypes[$table_name] : '';
						$insertData['order_id'] = $order_id;
						$insertData['tag_type'] = 'sms_tag';
						$insertData['tag'] = 'SMS';
...
public function sendEmailFromSparkpostApi($postData, $leadData = array()){
		...
		if(!empty($sparkPostDetail) && $sparkPostDetail[0]->type == 1){
			...
			//$page_url = '/about-us/fort-collins';
			//echo $page_url; die;
			...
			if(!empty($formInstance)){
				...
				if(!empty($formModelDetail)){
					...
					if(!empty($connectedTypesArr) && in_array('Sparkpost Email' , $connectedTypesArr)){
						if(!empty($formModelDetail[0]->sparkpost_mail_flow_id)){
							...
							if(!empty($sparkpost_mail_flow)){
								...
								$sparkPostUserArr = array();
								$sparkPostUserArr['name'] = (isset($formData['name']) && !empty($formData['name'])) ? $formData['name'] : '';
								$sparkPostUserArr['email'] =  (isset($formData['email']) && !empty($formData['email'])) ? $formData['email'] : '';
								$sparkPostUserArr['lead_type'] = (isset($leadData['lead_type']) && !empty($leadData['lead_type'])) ? $leadData['lead_type'] : '';
								$sparkPostUserArr['lead_id'] = (isset($leadData['lead_id']) && !empty($leadData['lead_id'])) ? $leadData['lead_id'] : '';
								$sparkPostUserArr['mail_flow_id'] = $sparkpost_mail_flow[0]->id;
								
								$mail_template_type = ($sparkPostUserArr['lead_type'] == "trial_forms") ? 'paid_trial_purchased' : 'day_1';
								$sparkPostUserArr['mail_template_type'] = $mail_template_type;
								$sparkPostUserArr['email_flows_status'] = 'pending';
								$sparkPostUserArr['created'] = date('Y-m-d H:i:s');
								$sparkPostUserArr['last_updated_date'] = date('Y-m-d H:i:s');
								...
								if(!empty($sparkPostUserArr['email']) && !empty($sparkpost_mail_flow[0]->id)){
									...
									if(empty($sparkpost_user_detail)){
										...
										$sparkpost_user_id = $this->db->insert_id();
									...
									if(!empty($sparkpost_user_id)){
										
										$this->db->where('status',1);
										$this->db->where('template_type',$mail_template_type);
										$sparkpost_mail_template = $this->query_model->getBySpecific('tbl_sparkpost_mail_templates','mail_flow_id',$sparkpost_mail_flow[0]->id);
...
public function getTotalWebsiteOptinLeads($start_date,$end_date,$result_type,$search_query = array()){
	
	$website_leads = array();
	$website_leads['email_opt_in_leads'] = array();
	$website_leads['birthday_parties_leads'] = array();
	...
	$website_leads['email_opt_in_leads'] = $query->result();
	...
	$website_leads['birthday_parties_leads'] = $query->result();
	
	
	
	$leads = array();
	$filterd_leads = array();
	$custom_filterd_leads = array();
		if(isset($website_leads) && !empty($website_leads)){
			
			foreach($website_leads as $key=> $lead_data){
				$a = 0;
				if(!empty($lead_data)){
					foreach($lead_data as  $lead){
						...
						$filterd_leads[$lead->email][$unique_id] = $lead;
						$filterd_leads[$lead->email][$unique_id]->lead_type = $lead_type;
						
						if($lead_type == "birthday_parties" || $lead_type == "contact_us" ){
							$filterd_leads[$lead->email][$unique_id]->created = $lead->date_added;
							$filterd_leads[$lead->email][$unique_id]->offer_type = "";
							$filterd_leads[$lead->email][$unique_id]->trans_status = "";
							if($lead_type == "contact_us"){
								if(!empty($lead->school)){
									$this->db->select(array('id','name'));
									$locationDetail = $this->query_model->getBySpecific('tblcontact','name',$lead->school);
									if(!empty($locationDetail)){
										$filterd_leads[$lead->email][$unique_id]->location_id = $locationDetail[0]->id;
									}
								}
								
							}
						}elseif($lead_type == "dojocart"){
							$filterd_leads[$lead->email][$unique_id]->location_id = $lead->location;
						}
						
						$custom_filterd_leads[$lead->email][$unique_id]['created'] = $created;
						$custom_filterd_leads[$lead->email][$unique_id]['email'] = $lead->email;
						$custom_filterd_leads[$lead->email][$unique_id]['unique_id'] = $unique_id;
					$a++;
					}
				}
		...
		if(isset($custom_filterd_leads) && !empty($custom_filterd_leads)){
			foreach($custom_filterd_leads as $lead_data){
			...
			if(!empty($orderLeadsSortEmail)){
				foreach($orderLeadsSortEmail as $email => $newlead_data){
					$lead = array();
					$lead = isset($filterd_leads[$newlead_data['email']][$newlead_data['unique_id']]) ? $filterd_leads[$newlead_data['email']][$newlead_data['unique_id']] : '';
					
					if(!empty($lead)){
						$created_date = strtotime($lead->created);
						$all_leads[$created_date] = $lead;
					}
			...
			$data['total_unique_leads'] = count($all_leads);
			//$data['latest_leads'] = array_slice($all_leads, 0, 5);
			
			if($result_type == "total_count_with_record"){
				$data['latest_leads'] = $all_leads;
			}
...
public function getVideoThumbnilImage($video_data){
	...
	if(!empty($video_data)){
		...
		if($video_data['video_img_type'] == "upload_image"){
			if(!empty($video_data['custom_video_thumbnail'])){
				$cover_image = base_url().'upload/class_schedule/'.$video_data['custom_video_thumbnail'];
			}
		}else{
	...
	public function getAllPagesListForSmsFlows($form_type_id){
...
public function checkSparkpostApiIsActive(){
	$result = 0;
	$this->db->select(array('type'));
	$sparkpostMailApi = $this->query_model->getBySpecific('tbl_sparkpost_mail','id',1);
	
	if(!empty($sparkpostMailApi)){
		if($sparkpostMailApi[0]->type == 1){
			$result = 1;
		}
	}
...
public function getNewNotification(){
	...
	$notifications = array();
	//$message_nofitications = array();
	//$lead_nofitications = array();
	
	$twilio_chat_api_is_active = $this->query_model->checkTwilioApiIsActive();
		
	if($twilio_chat_api_is_active == 1){
		
		$single_messages = $this->db->query("SELECT id,name,email,phone,message,msg_created,total_msgs FROM new_message_records where total_msgs = 1 order by `msg_created` desc")->result();
		if(!empty($single_messages)){
			foreach($single_messages as $message){
				$msg_created = strtotime($message->msg_created);
				$message->lead_type = "twilio_message";
				$notifications[$msg_created] = $message;
			}
		}
		
		$multi_messages = $this->db->query("SELECT id,name,email,phone,msg_created,total_msgs  FROM new_message_records where total_msgs > 1 order by `msg_created` desc")->result();
		
		if(!empty($multi_messages)){
			foreach($multi_messages as $message){
				$message->lead_type = "twilio_message";
				$msg_created = strtotime($message->msg_created);
				$notifications[$msg_created] = $message;
			}
	...
	/////////////// unique leads for notification /////
	$lead_status = '';
	$tags = '';
	$search_query = array('kanban_status_id' => $lead_status,'tags' => $tags);
	$notfiy_end_date =  date('Y-m-d');					
	$notfiy_start_date = date("Y-m-d", strtotime("-30 days",strtotime($notfiy_end_date)));
		...
		$notify_leads = array();
		$notify_filterd_leads = array();
		
		
		$notify_leads['email_opt_in_leads'] = $this->notifyOrderLeads($notfiy_start_date,$notfiy_end_date,'email_opt_in',$location,$search_query);
		
		$notify_leads['free_trial_leads'] = $this->notifyOrderLeads($notfiy_start_date,$notfiy_end_date,'free_trial',$location,$search_query);
		
		$notify_leads['paid_trial_leads'] = $this->notifyOrderLeads($notfiy_start_date,$notfiy_end_date,'paid_trial',$location,$search_query);
		
		$notify_leads['upsell_trial_leads'] = $this->notifyOrderLeads($notfiy_start_date,$notfiy_end_date,'upsell_trial',$location,$search_query);
	
		$notify_leads['birthday_parties_leads'] = $this->notifyBirthdayPartiesLeads($notfiy_start_date,$notfiy_end_date,$search_query);
		
		//$notify_leads['contact_us_leads'] = $this->notifyContactUsLeads($notfiy_start_date,$notfiy_end_date,$location,$search_query);
		
		$notify_leads['dojocart_leads'] = $this->notifyDojocartLeads($notfiy_start_date,$notfiy_end_date,$location,$search_query);
		
		$notify_custom_filterd_leads = array();
		if(isset($notify_leads) && !empty($notify_leads)){
			
			foreach($notify_leads as $key=> $lead_data){
				$a = 0;
				if(!empty($lead_data)){
					foreach($lead_data as  $lead){
						$lead_type = str_replace('_leads','',$key);
						...
						$notify_filterd_leads[$lead->email][$unique_id] = $lead;
						$notify_filterd_leads[$lead->email][$unique_id]->lead_type = $lead_type;
						
						if($lead_type == "birthday_parties" || $lead_type == "contact_us" ){
							$notify_filterd_leads[$lead->email][$unique_id]->created = $lead->date_added;
							$notify_filterd_leads[$lead->email][$unique_id]->offer_type = "";
							$notify_filterd_leads[$lead->email][$unique_id]->trans_status = "";
							
						}
						$notify_custom_filterd_leads[$lead->email][$unique_id]['created'] = $created;
						$notify_custom_filterd_leads[$lead->email][$unique_id]['email'] = $lead->email;
						$notify_custom_filterd_leads[$lead->email][$unique_id]['unique_id'] = $unique_id;
					$a++;
					}
				}
		...
		if(isset($notify_custom_filterd_leads) && !empty($notify_custom_filterd_leads)){
			foreach($notify_custom_filterd_leads as $lead_data){
			...
			if(!empty($orderLeadsSortEmail)){
				foreach($orderLeadsSortEmail as $email => $newlead_data){
					$lead = array();
					$lead = isset($notify_filterd_leads[$newlead_data['email']][$newlead_data['unique_id']]) ? $notify_filterd_leads[$newlead_data['email']][$newlead_data['unique_id']] : '';
					
					if(!empty($lead)){
						$created_date = strtotime($lead->created);
						$notifications[$created_date] = $lead;
					}
...
public function notifyOrderLeads($start_date,$end_date,$lead_type,$location = '',$search_query){
	
	$where = 'is_delete = 0 and is_read_notification = 0 and ';
	...
	$query = $this->db->query("select * from (select DISTINCT email,id,name,last_order_id,offer_type,trans_status,is_unique_trial,trial_id,created,is_delete,kanban_status_id,is_read_notification from tblorders order by id desc) as orders where ".$where);
...
public function notifyBirthdayPartiesLeads($start_date,$end_date,$search_query){
	...
	$query = $this->db->query("select * from (select DISTINCT email,id,name,date_added,is_delete,kanban_status_id,is_read_notification from tblbirthdayparty order by id desc) as orders where ".$where);
...
public function notifyContactUsLeads($start_date,$end_date,$location = '',$search_query){
	...
	$query = $this->db->query("select * from (select DISTINCT email,id,name,date_added,is_delete,kanban_status_id,is_read_notification from tblcontactusleads order by id desc) as orders where ".$where);
...
public function notifyDojocartLeads($start_date,$end_date,$location = '',$search_query){
	...
	$query = $this->db->query("select * from (select DISTINCT email,id,name,product_id,offer_type,amount,trans_status,created,is_delete,kanban_status_id,is_read_notification from tbl_dojocart_orders order by id desc) as orders where ".$where);
