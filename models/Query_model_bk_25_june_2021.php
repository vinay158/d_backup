					...
					if(!empty($phone) && !empty($message) && !empty($twilio_user_id)){
						...
						try{
						  ...
						  if(!empty($user_detail) && !empty($user_detail[0]->chat_conversation_sid) ){
						     //echo $user_detail[0]->chat_conversation_sid; die;
							//echo $reciever_by; die;
							
							try{
								/*$msg_record_student = $twilio->conversations->v1->conversations('CHd0746b4bd94c4f57b5dec06012c09eb6')
                                     ->messages("IM55af8d352c5146d6b42505fbbe344c12")
                                     ->fetch();*/
									 
								/*$msg_record_admin = $twilio->conversations->v1->conversations('CHd0746b4bd94c4f57b5dec06012c09eb6')
                                     ->messages("IMcadaeaa2b6694634b3c223578e05ffb4")
                                     ->fetch(); */
									 
								$msg_record_student_chatboard = $twilio->conversations->v1->conversations('CHd0746b4bd94c4f57b5dec06012c09eb6')
                                     ->messages("IM85a9cc537b374018adc7193e4975c21e")
                                     ->fetch();
									 
									 
								//echo '<pre>msg_record_student'; print_r($msg_record_student); 
								//echo '<pre>msg_record_admin'; print_r($msg_record_admin); 
								echo '<pre>msg_record_student_chatboard'; print_r($msg_record_student_chatboard); 
								
								die;
								
							}catch (Exception $e) {
							
								echo 'Msg record Caught exception --->: ',  $e->getMessage(), "\n"; die;
							}
								
							if($reciever_by == "admin"){
								try{
									$participant = $twilio->conversations->v1->conversations($user_detail[0]->chat_conversation_sid)
											 ->participants
											 ->create([
														  "identity" => "realEstateAgentsss",
														  "messagingBindingProjectedAddress" => '+13472270477'
													  ]
											 );
								...
								$participant_new = $twilio->conversations->v1->conversations($user_detail[0]->chat_conversation_sid)
												 ->participants
												 ->create([
									...
									echo '<pre>participant new==>'; print_r($participant_new); 
							}catch (Exception $e) {
							
										echo 'Admin prar Caught exception --->: ',  $e->getMessage(), "\n"; 
									}
							...
							$message = $twilio->conversations->v1->conversations($user_detail[0]->chat_conversation_sid)
										...
										echo 'Msg Caught exception --->: ',  $e->getMessage(), "\n"; die;
									}
					...
					if(!empty($twilio_user_id)){
						...
						/******** SEND ADMIN SMS ****/	
							...
							if(!empty($admin_twilio_sms_flows)){
								...
								$admin_phone = $twilioChatApi[0]->twilio_admin_phone_number;
								$admin_author_name = 'user_'.$twilio_user_id;
								
								$adminMsgData = array('twilio_user_id' => $twilio_user_id,'reciever_by'=>'admin','phone'=>$admin_phone,'msg_type'=>'student_to_admin','message'=>$admin_msg_template,'twilio_sms_msg_id'=>$admin_twilio_sms_msg_id,'author_name'=>$admin_author_name,'twilio_user_name'=>$twilioUserArr['name']);
								
							//	echo '<pre>adminMsgData'; print_r($adminMsgData); 
									...
									if(!empty($messages)){
										...
										foreach ($messages as $msg) {
											...
											if(empty($msg_exit)){
												...
												$msgData = array('twilio_user_id' => $twilio_user_id,'reciever_by'=>$reciever_by,'phone'=>$phone,'msg_type'=>$msg_type,'message'=>$msg->body,'twilio_sms_msg_id'=>$twilio_sms_msg_id,'author_name'=>$author_name);
