...
class Cartorder_model extends CI_Model {
	...
	function addTrialNew_($data, $product_id){
		...
		*/				
		...
		$extraContentArr  = array('dojocart_title' => $product_title,'dojocart_custom_fields'=>$dojocart_custom_fields);
		...
		$paypalEmail = 'info@websitedojo.com';
			...
			$insertOrder['location'] = $location_data['id'];
			...
			if(!empty($order_id)){
				...
				$this->query_model->updateOrderForKabanLeads($order_id,'tbl_dojocart_orders','Dojo Cart Opt-in',$_POST);
...
/********* Ends CODE FOR ADMIN PHONE MESSAGES ******/
	...
	function saveNewDojocartForms($data, $product_id,$pdfLink = null){
		...
		*/				
		...
		$extraContentArr  = array('dojocart_title' => $product_title,'dojocart_custom_fields'=>$dojocart_custom_fields,'pdfLink'=>$pdfLink);
		$emailAutoResponder = $this->query_model->checkFormModuleAutoResponder($_POST,$extraContentArr);
//echo '<pre>emailAutoResponder'; print_r($emailAutoResponder); die;
		...
		$paypalEmail = 'info@websitedojo.com';
			...
			$insertOrder = array();
			$insertOrder['name'] = isset($_POST['name']) ? $_POST['name'] : '';
			$insertOrder['last_name'] =isset($_POST['last_name']) ? $_POST['last_name'] : '';
			$insertOrder['email'] =isset($_POST['email']) ? $_POST['email'] : '';
			...
			//$insertOrder['product_id'] = isset($_POST['product_id'])? $_POST['product_id'] : 0;
			$this->query_model->insertData('tbl_dojocart_orders', $insertOrder);	
//echo '<pre>insertOrder'; print_r($insertOrder); die;
			...
			/*********Start CODE FOR ADMIN PHONE MESSAGES ******/
			$is_unique_dojocart = 0;
			$unique_email_address = '';
			if(isset($_POST['is_unique_dojocart']) && $_POST['is_unique_dojocart'] == 1){
				if(!empty($productDetail)){
					if(!empty($productDetail[0]->unique_email_address)){
						$is_unique_dojocart = 1;
						$unique_email_address = $productDetail[0]->unique_email_address;
					}
				}
				//echo '<pre>productDetail'; print_r($productDetail); die;
			}
			...
			if(isset($emailAutoResponder['admin_email'])){
				...
				if(!empty($text_address) && $is_unique_dojocart == 0){	
...
/********* Ends CODE FOR ADMIN PHONE MESSAGES ******/
				...
				//Mail to admin	
					...
					$site_email = (!empty($unique_email_address) && $is_unique_dojocart = 1) ? $unique_email_address : $site_email;
				...
				if(isset($emailAutoResponder['customer_email']) && $emailAutoResponder['customer_email'] == 1 && !empty($email)){
