							...
							foreach($trial_offers as $trial_offer){
								if($trial_offer->trial == 1){
									...
									if(!empty($payment)){
										...
										if($payment[0]->authorize_net_payment == 1 || $payment[0]->braintree_payment == 1 || $payment[0]->stripe_payment == 1 || $payment[0]->stripe_ideal_payment == 1){
	  ...
	  $this->db->from('tblcategory');
		 ...
		 $className = ($parent_id == 0) ? "mainCat" : 'subCat';
		  if(!empty($result)){
             ...
             $str .= '<li   id="menu_'.$cat->cat_id.'">';
			 
             $str .= '<div class="categories-link " id="catid_'.$cat->cat_id.'">';
			...
			if($cat->cat_id == $this->uri->segment(4)){
			 ...
			 $str .= '<a href="admin/'.$linkType.'/view/'.$cat->cat_id.'" id ="cat'.$cat->cat_id.'" class="show-entries '.$activeClass.'" style="color:'.$publishedClr.'">'.$dashes.$cat->cat_name.'</a><span></span>';
			  
			  if(($cat->cat_id != 25) && ($cat->cat_id != 26)) {
					$str .= '<span><a  href="javascript:void(0)" class=" full_alternate_popup" data-toggle="modal" data-target="#fullAlternatePopup" action_type="edit" item_id="'.$cat->cat_id.'"  table_name="tblcategory" form_type="full_width_row">Edit</a></span>';
										
			 }
			 
			  if(($cat->cat_id != 25) && ($cat->cat_id != 26)) {
				  $str .= '<small><a class="delete_item" data-toggle="modal" data-target="#popupDeleteItem" item_id="'.$cat->cat_id.'"   table_name="tblcategory" item_title="'.$cat->cat_name.'" section_type="full_width">Delete<input type="hidden" name="post-perm" class="post-perm" value="'.$cat->permission.'"/></a></small>';
			  }
