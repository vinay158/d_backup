...
public function mwalteSpIdlPymSvdj(){
	...
	public function paypal_payment_gateway(){
		...
		if(isset($_POST['submit'])){
			...
			setcookie($cookie_name, $cookie_value, time() + (86400 * 30), "/"); // 86400 = 1 day
					...
					$redirect_urls->setReturnUrl(base_url().'payment/paypal_success') /** Specify return URL **/
				...
				if(!empty($payment->getLinks())){
					...
					$orderRecord = array('order_type'=>'trial_offer','order_id'=>$order_id,'created_date'=>date('Y-m-d H:i:s'));
					...
					//echo '<pre>payer=>'; print_r($payment); 
	...
	public function paypal_success(){
		...
		if(!empty($payment_id) &&  !empty($payer_id)){
			...
			if(isset($result->transactions[0]->custom) && !empty($result->transactions[0]->custom)){
				...
				/********* Ends CODE FOR ADMIN PHONE MESSAGES ******/				
					...
					//$results['error_message'] = "<h1 class='payment_result'>Payment Received!</h1><p class='payment_result'>A representative from our school will contact you shortly with more information to start your trial.</p>";
						$_POST['order_id'] = $order_id;
						$thankYouPageDetail = $this->query_model->getFormModuleThankYouPage($_POST, 'paid');
						//echo '<pre>thankYouPageDetail'; print_r($thankYouPageDetail); die;
						//redirect(@base_url().'starttrial/thankyou');
						$trial_offer_thankyou_url = $this->query_model->getTrialOfferThankyouUrl($_POST);
						redirect(@base_url().$trial_offer_thankyou_url);
			...
			/*echo '<pre>_POST'; print_r($_POST); die;
			echo 'success';
			echo '<pre>apiContext'; print_r($apiContext);
			echo '<pre>payment'; print_r($payment);
			echo '<pre>execution'; print_r($execution);
			echo '<pre>result'; print_r($result->transactions[0]->custom);
