...
public function cron_for_twilio_sms_flow_msgs(){
	...
	if(!empty($twilio_users)){
		...
		$this->db->where("msg_type !=","template_1");
		...
		foreach($twilio_users as $twilio_user){
			...
			if($twilio_user->sms_flows_status == "pending"){
				...
				$this->db->where("msg_type","template_1");
				$first_twilio_sms_flow = $this->query_model->getByTable('tbl_twilio_sms_flows');
				
				$first_flow_start_time = $first_twilio_sms_flow[0]->start_time;
				$first_current_time = date("H:i"); 
				
				/*$first_flow_start_time = "07:00";
				$first_current_time = "07:00";*/
				/******* sending first hold msg *********/
				
				if($first_flow_start_time <= $first_current_time){
					
					$this->db->where('template_msg_type','template_1');
					...
					if(!empty($hold_template_1_msgs)){
						//echo '<pre>hold_template_1_msgs'; print_r($hold_template_1_msgs); die;
					...
					foreach($twilio_sms_flows as $twilio_sms_flow){
						...
						if(empty($pending_sms_flows)){
							...
							if(!empty($user_last_template_msg)){
								...
								/*$next_msg_created_date = "2020-11-12";
								$current_date = "2020-11-12";
								$flow_start_time = "07:00";
								$current_time = "07:00";
								echo 'last_msg_created_date=>'.$next_msg_created_date.'<br/>';
								echo 'current_date=>'.$current_date.'<br/>';
								echo 'flow_start_time=>'.$flow_start_time.'<br/>';
				...
				echo "Cron runned:  no users for pending sms flows";
			}
		}
	}
