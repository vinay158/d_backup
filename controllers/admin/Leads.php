...
public function ajax_spam_lead_info(){
		...
		if(isset($_POST['action']) && $_POST['action'] == "get_record"){
			...
			if(!empty($lead_id) && !empty($email)){
				$this->db->where('email',$email);
				$data['spam_lead'] = $this->query_model->getBySpecific('tbl_sp_leads','id',$lead_id);
			}
		}
		
		$this->load->view('admin/ajax_spam_lead_info',$data);
}
