...
class Leads extends CI_Controller {
	...
	public function index()
	{
		...
		if($lead_type == "all" || $lead_type == "email_opt_in"){
			$data['leads']['email_opt_in_leads'] = $this->getOrderLeads($data['start_date'],$data['end_date'],'email_opt_in',$location);
		}
		
		if($lead_type == "all" || $lead_type == "free_trial"){
			$data['leads']['free_trial_leads'] = $this->getOrderLeads($data['start_date'],$data['end_date'],'free_trial',$location);
		}
		
		if($lead_type == "all" || $lead_type == "paid_trial"){
			$data['leads']['paid_trial_leads'] = $this->getOrderLeads($data['start_date'],$data['end_date'],'paid_trial',$location);
		}
		
		if($lead_type == "all" || $lead_type == "upsell_trial"){
			$data['leads']['upsell_trial_leads'] = $this->getOrderLeads($data['start_date'],$data['end_date'],'upsell_trial',$location);
		}
		
		if($lead_type == "all" || $lead_type == "birthday_parties"){
			$data['leads']['birthday_parties_leads'] = $this->getBirthdayPartiesLeads($data['start_date'],$data['end_date']);
		}
		...
		if($lead_type == "all" || $lead_type == "dojocart"){
			$data['leads']['dojocart_leads'] = $this->getDojocartLeads($data['start_date'],$data['end_date'],$location);
		}
...
public function getOrderLeads($start_date,$end_date,$lead_type,$location = ''){
	...
	$query = $this->db->query("select * from (select DISTINCT email,id,name,last_name,phone,location_id,last_order_id,offer_type,trans_status,is_unique_trial,trial_id,created,is_delete from tblorders order by id desc) as orders where ".$where." GROUP by orders.email");
	$leads = $query->result();
...
public function getBirthdayPartiesLeads($start_date,$end_date){
	
	$where = 'is_delete = 0 and ';
	$where .= "DATE_FORMAT(date_added,'%Y-%m-%d') >= '".$start_date."' and ";
	$where .= "DATE_FORMAT(date_added,'%Y-%m-%d') <= '".$end_date."'";
	
	$query = $this->db->query("select * from (select DISTINCT email,id,name,last_name,phone,location_id,party_date,guests,date_added,is_delete from tblbirthdayparty order by id desc) as orders where ".$where." GROUP by orders.email");
	$leads = $query->result();
...
public function getContactUsLeads($start_date,$end_date,$location = ''){
	...
	$query = $this->db->query("select * from (select DISTINCT email,id,name,last_name,phone,school,message,date_added,is_delete from tblcontactusleads order by id desc) as orders where ".$where." GROUP by orders.email");
	$leads = $query->result();
...
public function getDojocartLeads($start_date,$end_date,$location = ''){
	...
	$query = $this->db->query("select * from (select DISTINCT email,id,name,last_name,phone,location,product_id,offer_type,amount,trans_status,created,is_delete from tbl_dojocart_orders order by id desc) as orders where ".$where." GROUP by orders.email");
	$leads = $query->result();
...
function exportorderleads(){
		...
		$file = fopen('php://output', 'w');                              
      	fputcsv($file, array('Name', 'Last Name', 'Phone', 'Email', 'Program', 'School','Trial Name','Trial Type','Upsell','Upsell Name','Coupon Code','Coupon Discount', 'Amount', 'Status','Page Url', 'Date',"Child's Name","Child's Age"));
		...
		foreach($records as $r){
			...
			//fputcsv($file, array('Name', 'Phone', 'Email', 'Program', 'School','Trial Name','Trial Type','Upsell','Upsell Name','Coupon Code','Coupon Discount', 'Amount', 'Status','Page Url', 'Date'));
			 ...
			 $record = array($name,$last_name, $phone, $email, $program, $school,$offer_title,$offer_type,$upsell,$upsale_title,$coupon_code,$coupon_discount, $amount,$status,$page_url, $date,$child_name,$child_age);
			fputcsv($file, $record);				
		}
		fclose($file);
		exit();	
