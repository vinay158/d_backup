...
class Kanban_leads extends CI_Controller {
	...
	//echo '<pre>orderLeadsSortCreatedDate'; print_r($orderLeadsSortCreatedDate); die;
		...
		if(!empty($orderLeadsSortCreatedDate)){
			foreach($orderLeadsSortCreatedDate as $created => $newlead_data){
			//	echo '<pre>created'; print_r($created); die;
				$lead = array();
				...
				if(!empty($lead)){
					...
					if(empty($sort_number) || $sort_number == 0){
						$sort_number = $this->query_model->createAndGetKanbanOrderSortNumber($lead->kanban_status_id,$new_lead_type,$lead->id,'onload',$created);
					}
	...
	//echo '<pre>'; print_r($data['all_leads']); die;
		...
		$is_mobile_number_linked_twilio = 0;
		...
		if(!empty($twilioChatApi) && $twilioChatApi[0]->type == 1){
			if(!empty($twilioChatApi[0]->sid) && !empty($twilioChatApi[0]->token) ){
				$is_mobile_number_linked_twilio = 1;
			}
		}
		$data['is_mobile_number_linked_twilio'] = $is_mobile_number_linked_twilio;
...
public function ajax_save_full_alternate_row(){
		...
		if(isset($searcharray['update'])){
				...
				if(!empty($item_id)){
					...
					$result['kanban_lead_status'] = $data['kanban_lead_status'];
...
public function update_move_lead_status_id(){
	...
	$action = (isset($_POST['action']) && !empty($_POST['action'])) ? $_POST['action'] : '';
	$lead_created = (isset($_POST['lead_created']) && !empty($_POST['lead_created'])) ? $_POST['lead_created'] : '';
	//echo 'new_kanban_status_id=>'.$new_kanban_status_id.'==>old_kanban_status_id=>'.$old_kanban_status_id; die;
	$data = array();
	$data['response'] = 0;
	if(!empty($lead_type) && !empty($lead_id) && !empty($new_kanban_status_id) && !empty($old_kanban_status_id) && $action == "update_lead_status"){
		...
		if(!empty($table_name)){
			...
			if($new_kanban_status_id != $old_kanban_status_id){
				...
				$this->query_model->createAndGetKanbanOrderSortNumber($new_kanban_status_id,$lead_type,$lead_id,'drag',$lead_created);
			}
...
public function ajax_sort_kanban_leads(){
	...
	$action = (isset($_POST['action']) && !empty($_POST['action'])) ? $_POST['action'] : '';
	//echo '<prE>responseData'; print_r($responseData); die;
	$data = array();
	$data['response'] = 0;
	if(!empty($responseData) && !empty($kanban_status_id) && $action == "sort_kanban_leads"){
		...
		foreach($sortPositions as $sort_position => $sort_data){
			...
			$insertData['sort_type'] = 'drag';
			$insertData['created'] = $sort_data['lead_created'];
...
public function delete_kanban_status(){
	parse_str($_POST['formData'], $searcharray);
...
public function ajaxSaveLeadProfileImage(){
		...
		if(isset($_FILES['file']['name']) && !empty($_FILES['file']['name'])){
			//$_FILES['file']['name'] = time().$_FILES['file']['name'];
				...
				$config['upload_path'] = 'upload/kanban_user_profiles/';
				...
				$resize_config['source_image'] = 'upload/kanban_user_profiles/'.$file;
				...
				$resize_config['new_image'] = 'upload/kanban_user_profiles/thumb/'.$file;
				...
				if($image_info['width']  >= 150){				
					$new_width = 150;
					$new_height = round((150/$image_info['width'])*$image_info['height']);				
				...
				$this->query_model->tinyImageCampressAndResize('upload/kanban_user_profiles/'.$file);
				
				$this->query_model->tinyImageCampressAndResize('upload/kanban_user_profiles/thumb/'.$file);
				
				if(!empty($file) && isset($_POST['email']) && !empty($_POST['email'])){
					$lead_email = $_POST['email'];
					
					$record = $this->query_model->getBySpecific('tbl_lead_profile_img','email',$lead_email);
					
					if(!empty($record)){
						
						$updateData['image']  = $file;
						$updateData['created']  = date('Y-m-d H:i:s');
						$this->query_model->update('tbl_lead_profile_img', $record[0]->id, $updateData);
					}else{
						$insertData['email']  = $lead_email;
						$insertData['image']  = $file;
						$insertData['created']  = date('Y-m-d H:i:s');
						$this->query_model->insertData('tbl_lead_profile_img',$insertData);
					}
