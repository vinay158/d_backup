...
public function ajax_send_twilio_new_sms_popup_form(){
		...
		if(isset($_POST['action_type']) && !empty($_POST['action_type'])){
			...
			/*$new_lead_type = $this->query_model->getKanbanLeadTypeToOrderType($records['lead_type']);
			$main_lead_detail =  $this->getMainLeadById($records['lead_id'],$records['lead_type']);
			
			if($new_lead_type == "trial_offer_lead"){
				if(isset($main_lead_detail['page_url']) && !empty($main_lead_detail['page_url'])){
...
public function ajax_send_twilio_new_sms(){
	...
	parse_str($_POST['formData'], $searcharray);
		...
		if(isset($searcharray['update'])){
				...
				$lead_email = isset($searcharray['lead_email']) ? trim($searcharray['lead_email']) : '';
					...
					if(empty($twilio_user_detail)){
						...
						if(!empty($orderDetail)){
							if($lead_table_name == "tblorders"){
								if($orderDetail[0]->offer_type == "Paid"){
									$twilio_user_lead_type = 'trial_forms';
									$twilio_sms_template = 'paid_trial_template';
								}else{
						...
						$twilioUserArr['email'] = $lead_email;
						...
						$twilioUserArr['sms_flow_id'] = 0;
						$twilioUserArr['msg_template_type'] = $twilio_sms_template;
						$twilioUserArr['lead_created'] = date('Y-m-d H:i:s');
