...
class Kanban_leads extends CI_Controller {
	...
	/*if(isset($data['filterd_leads']) && !empty($data['filterd_leads'])){
		krsort($data['filterd_leads']);
	}*/
	
	//echo '<pre>'; print_r($data['filterd_leads']); die;
	/*	$data['new_all_leads'] = array();
		$data['all_leads'] = array();
			...
			foreach($data['filterd_leads'] as $email => $lead_data){
				...
				foreach($lead_data as $key => $lead){
					if(strtotime($lead->created) > $last_date){
						...
						$data['new_all_leads'][$lead->kanban_status_id][$lead->email] = $lead;
					}
				}
			}
		}
		//echo '<pre>all_leads==>'; print_r($data['new_all_leads']); die;
		if(isset($data['kanban_lead_status']) && !empty($data['kanban_lead_status'])){
			foreach($data['kanban_lead_status'] as $status){
				if(isset($data['new_all_leads'][$status->id]) && !empty($data['new_all_leads'][$status->id])){
					ksort($data['new_all_leads'][$status->id]);
					$data['all_leads'][$status->id] = $data['new_all_leads'][$status->id];
				}
				
			}
		}*/
	//echo '<pre>all_leads==>'; print_r($data['all_leads']); die;
