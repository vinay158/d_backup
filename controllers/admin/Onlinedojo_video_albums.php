...
class Onlinedojo_video_albums extends CI_Controller {	
	...
	if($media_url){
		...
		'video_img_type'=>$video_img_type,'custom_video_thumbnail'=>$custom_video_thumbnail);		
	}else{
		$args = array( "desc" => $media_desc,'video_img_type'=>$video_img_type,'custom_video_thumbnail'=>$custom_video_thumbnail);
	}		
	if( $album_cover == 1 ):
