...
class Twilio_smsflow extends CI_Controller {
	...
	public function index(){
		...
		if(!empty($is_logged_in) && $is_logged_in == true){
			...
			$data['sparkpost_mail_flows'] = $this->query_model->getByTable('twilio_sms_flows');
			...
			if(!empty($data['sparkpost_mail_flows'])){
				foreach($data['sparkpost_mail_flows'] as $sparkpost_flow){
					...
					$data['sparkpost_templates']['days_template'][$sparkpost_flow->id] = $this->query_model->getbySpecific('tbl_twilio_sms_templates', 'sms_flow_id',$sparkpost_flow->id);
					
					if($sparkpost_flow->id == 1){
						$this->db->order_by('pos','ASC');
						$this->db->select(array('id','title','sms_flow_id','status','pos','created','template_type'));
						$this->db->where('template_type','paid_trial_template');
						$this->db->or_where('template_type','admin_sms_template');
						$data['sparkpost_templates']['other_templates'][$sparkpost_flow->id] = $this->query_model->getbySpecific('tbl_twilio_sms_templates', 'sms_flow_id',$sparkpost_flow->id);
					}
