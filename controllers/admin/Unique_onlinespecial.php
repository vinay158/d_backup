...
class Unique_onlinespecial extends CI_Controller {
	...
	function __construct(){
		...
		$this->load->model("unique_onlinespecial_model");
	}
	
	public function index()
	{
		redirect("admin/unique_onlinespecial/view/1");
	}
	...
	public function view(){
		...
		if(!empty($is_logged_in) && $is_logged_in == true){
			...
			$data['link_type'] = "unique_onlinespecial"; //cotroller name
			
			$this->db->select(array('id','name'));
			$this->db->where("type", 'trial_offer');
			$data['category_detail'] = $this->query_model->getbySpecific('tbl_unique_onlinespecial_categories',"id",$this->uri->segment(4));
			...
			if(empty($data['category_detail'])){
				...
				$this->db->where("type", 'trial_offer');
				$data['category_detail'] = $this->query_model->getbyTable('tbl_unique_onlinespecial_categories');
			...
			$data['offer'] = $this->query_model->getbyTable("tbl_unique_specialoffer");
			...
			$this->db->order_by("pos", 'asc');
			$data['unique_trial_boxes'] = $this->db->get("tbl_unique_onlinespecial_categories")->result();
			
			$this->db->where("type", 'trial_offer');
			$this->db->order_by("pos", 'asc');
			$this->db->select(array('id','name','slug','heading'));
			$data['categories'] = $this->db->get("tbl_unique_onlinespecial_categories")->result();
			
			$this->db->select(array('id','cat_id','title'));
			$allTrialOffers = $this->query_model->getByTable('tbl_unique_specialoffer');
			
			if(!empty($allTrialOffers)){
				foreach($allTrialOffers as $offer){
					$this->db->select(array('id'));
					$trialCat = $this->query_model->getBySpecific('tbl_unique_onlinespecial_categories','id',$offer->cat_id);
					if(empty($trialCat)){
						$this->db->where("id", $offer->id);
						$this->db->delete("tbl_unique_specialoffer");
					}
				}
			}
			
			
			$this->load->view("admin/unique_onlinespecial_index", $data);
	...
	public function update_unique_trial_box(){
		if(isset($_POST['update_unique_trial_box']) && $_POST['update_unique_trial_box'] == "Update"){
			if(isset($_POST['unique_trial_box']) && !empty($_POST['unique_trial_box'])){
				$this->query_model->deletebySpecific('tbl_unique_onlinespecial_categories','type','unique_trial_box');
				foreach($_POST['unique_trial_box'] as $unique_trial_box){
					$insertData = array();
					$insertData['name'] = isset($unique_trial_box['name']) ? $unique_trial_box['name'] : '';
					$insertData['heading'] = isset($unique_trial_box['heading']) ? $unique_trial_box['heading'] : '';
					$insertData['box_web_special'] = isset($unique_trial_box['box_web_special']) ? $unique_trial_box['box_web_special'] : '';
					$insertData['box_button_text'] = isset($unique_trial_box['box_button_text']) ? $unique_trial_box['box_button_text'] : '';
					$insertData['box_button_url'] = isset($unique_trial_box['box_button_url']) ? $unique_trial_box['box_button_url'] : '';
					$insertData['pos'] = (isset($unique_trial_box['pos']) && !empty($unique_trial_box['pos'])) ? $unique_trial_box['pos'] : 0;
					$insertData['type'] = 'unique_trial_box';
					if(isset($unique_trial_box['program_id']) && !empty($unique_trial_box['program_id'])){
						$programsData = explode('~',$unique_trial_box['program_id']);
						if(isset($programsData[0]) && !empty($programsData[0])){
							$insertData['program_type'] = $programsData[0];
							$insertData['program_id'] = $programsData[1];
						}
					}
					
					$this->query_model->insertData('tbl_unique_onlinespecial_categories',$insertData);
				}
			} 
		}
	...
	public function delete_unique_trial_box(){
		
		if(count($_POST)>0){		
										
			$id = $_POST['unique_trial_box_id'];
			
			$this->db->where("id", $id);
			$this->db->where("type", 'unique_trial_box');
			if($this->db->delete("tbl_unique_onlinespecial_categories"))
			{					
				echo 1;
			}
			else
	...
	public function ajaxGetProgramsListByCat(){
		$programDropdown = '';
		if(isset($_POST['program_cat_id']) && !empty($_POST['program_cat_id'])){
			
			$programs = $this->query_model->programsListByCatId($_POST['program_cat_id']);
			$selected_program_id = (isset($_POST['selected_program_id']) && !empty($_POST['selected_program_id'])) ? $_POST['selected_program_id'] : '';
			//echo $selected_program_id; die;
			if(!empty($programs)){
				$programDropdown .= '<h1>Programs</h1><select name="unique_program_id" class="field"><option value=""> -Select- </option>';
				foreach($programs as $program){
					$selected = ($selected_program_id == $program->id) ? 'selected=selected' : '';
					$programDropdown .= '<option value="'.$program->id.'" '.$selected.'>'.$program->program.'</option>';
				}
				$programDropdown .= '</select>';
			}
		}
		
		echo  $programDropdown; die;
	...
	public function add(){
		...
		if(!empty($is_logged_in) && $is_logged_in == true){
			...
			if(isset($_POST['update'])):
				...
				$this->unique_onlinespecial_model->addOffer();
			endif;
			
			$this->load->view("admin/unique_onlinespecial_add", $data);
		}
		else{
			redirect("admin/login");
		}
	...
	public function edit(){
		...
		if(!empty($is_logged_in) && $is_logged_in == true){
			...
			$data['title'] = "Online Special";
			$data['details'] = $this->query_model->getbyId("tbl_unique_specialoffer", $this->uri->segment(4));
			...
			$data['upsells'] = $this->query_model->getbySpecific("tbl_unique_onlinespecial_upsales",'trial_offer_id',$this->uri->segment(4));
			
				if(isset($_POST['update'])):
				
					$this->unique_onlinespecial_model->updateOffer();
				endif;
			//	echo '<pre>'; print_r($data); die;
				$this->load->view("admin/unique_onlinespecial_edit", $data);
			}else{
				redirect($this->index());
			}
		}else{
	...
	public function delete_upsell(){
		...
		if(count($_POST)>0){		
			...
			if($this->db->query("delete from tbl_unique_onlinespecial_upsales where id=".$id.""))
			{					
				echo 1;
			}
			else
	...
	$this->db->query("UPDATE `tbl_unique_specialoffer` SET `pos`=" . $i . " WHERE `id`='" . $menu[$i] . "'") or die(mysqli_error($this->db->conn_id));
	}
	}
	...
	$this->db->where("id", $id);
	if($this->db->update("tbl_unique_specialoffer", array("published" => $pub)))
	{	
		echo 1;
	...
	//echo '<pre>'; print_r($_POST); die;
	$this->db->where("id", $id);
	if($this->db->update("tbl_unique_specialoffer", array("display_trial" => $display_trial)))
	{	
		echo 1;
	...
	$this->db->where("id", $id);
	if($this->db->delete("tbl_unique_specialoffer"))
	{
	redirect($this->index());
	}
	else
	...
	public function selectFrontDisplayTrial(){
			...
			$records = $this->query_model->getbySpecific('tbl_unique_specialoffer','trial',$trial);
			...
			if($trial == 0){
				if(count($records) >= 1){
					...
					$this->query_model->update("tbl_unique_specialoffer", $id, $data);
					echo '1';
				}
			} else {
				if(count($records) >= 2){
	...
	public function operateCategory(){
		if(isset($_POST['submit'])){
			...
			if($_POST['edit_id'] > 0){
				$this->query_model->updateData('tbl_unique_onlinespecial_categories','id',$_POST['edit_id'], $data);
			}else{
				$this->query_model->insertData('tbl_unique_onlinespecial_categories', $data);
			}
		}
		redirect("admin/onlinespecial");
	}
	...
	$this->db->query("UPDATE `tbl_unique_onlinespecial_categories` SET `pos`=" . $i . " WHERE `id`='" . $menu[$i] . "'") or die(mysqli_error($this->db->conn_id));
	}
	}
	...
	public function add_category(){
		...
		$data['multi_location'] = $multi_location[0]->field_value;
		
		
			
		$this->load->view("admin/add_cat_unique_onlinespecial", $data);
		...
		if(isset($_POST['update'])){
			...
			$insertData['program_cat_id'] = (isset($_POST['program_cat_id']) && !empty($_POST['program_cat_id'])) ? $_POST['program_cat_id'] : 0; 
			$insertData['unique_program_id'] = (isset($_POST['unique_program_id']) && !empty($_POST['unique_program_id'])) ? $_POST['unique_program_id'] : 0; 
			...
			redirect("admin/unique_onlinespecial/view/".$id);
		}
		}
	}
	...
	public function edit_category(){ 
		...
		if(!empty($is_logged_in) && $is_logged_in == true)
		{
			...
			$this->db->where('id',1);
			$multi_location = $this->query_model->getbyTable("tblconfigcalendar");
			$records['multi_location'] = $multi_location[0]->field_value;
		
			//echo '<pre>locations'; print_r($data['locations']); die;
			$this->db->where("id", $this->uri->segment(4));
			$records['pagedetails'] = $this->query_model->getbyTable('tbl_unique_onlinespecial_categories');
		...
		if(isset($_POST['update'])){
			...
			$this->query_model->updateData('tbl_unique_onlinespecial_categories','id',$this->uri->segment(4), $insertData);
			
			redirect("admin/unique_onlinespecial/view/".$this->uri->segment(4));
		}
		
		$this->load->view("admin/edit_cat_unique_onlinespecial", $records);
	...
	public function deleteCategoryImage(){
		...
		if(count($_POST)>0){			
			...
			$query = $this->db->query("update tbl_unique_onlinespecial_categories set left_photo='' where id=".$id);
			echo 1;
		}else{
				echo 0;
		}
	...
	public function add_promocodes(){
		...
		if(!empty($is_logged_in) && $is_logged_in == true)
		{
			...
			$records['trial_offers'] = $this->query_model->getbyTable('tbl_unique_specialoffer');
	...
	public function duplicate_trial_offer(){
		...
		if(isset($action) && $action == "duplicate_record"){
			...
			if(isset($item_id) && !empty($item_id) ){
				...
				$details = $this->query_model->getbySpecific('tbl_unique_specialoffer','id', $item_id);
				...
				if(!empty($details)){
					...
					$this->query_model->insertData('tbl_unique_specialoffer', $programArr);
					$duplicate_trialoffer_id = $this->db->insert_id();
					
					//$duplicate_program_id = 117;
					
					// saving  code other tables
					$tables = array('tbl_unique_onlinespecial_upsales');
			...
			echo 1;
			//redirect("admin/unique_onlinespecial/view/".$_POST['cat_id']);
		}
	}
