						...
						<div class="az-content-body ">
							...
							<div class="az-mail-header">
								<div>
								  ...
								  <p>You have <span class="total_alternating_full_width_row"><?php echo !empty($programs) ? count($programs) : 0; ?></span> entries</p>
								</div>
								<div>
								 <a href="admin/<?=$link_type?>/add" class="button_class btn btn-indigo ">Add Featured Program</a>
								</div>
							  </div>
							  
			<ul class="ajax_record_sortable alternating_full_width_row"  table_name="tblfeaturedprogram" >
					...
					<li   id="menu_<?=$row->id?>" class="full_width_row_<?=$row->id?> az-contact-info-header">
						<div class="manager-item media">
							<div style="float:left;">
								...
								<div class="imgbox"><img src="<?php echo !empty($row->photo_thumb) ? base_url().$row->photo_thumb : base_url().'assets_admin/img/no-image.png'?>" class="list_img"></div>
								
								<h4 class="full_width_row_heading_<?=$row->id?>"><a href="javascript:void(0)" ><?=$row->program_title.' ('.$row->p_type.')';?></a></h4>
							</div>
						...
						<nav class="nav">
							  ...
							  <a href="admin/featuredprograms/edit/<?=$row->id; ?>" class="badge badge-primary">Edit</a>
							  
									<a class="badge badge-primary ajax_record_delete" data-toggle="modal" data-target="#popupDeleteRecord" item_id="<?=$row->id;?>"   table_name="tblfeaturedprogram" item_title="<?=$row->program_title;?>" section_type="full_width">Delete</a>
									
									<a href="javascript:void(0)" id="unpub_<?=$row->id; ?>" class="ajax_record_publish"  table_name="tblfeaturedprogram"  is_new="0">
