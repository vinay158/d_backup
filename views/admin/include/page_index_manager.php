...
<div class="program_full_detail page-section new_lisiting_block default_template" id="AlternatingFullWidth">
					...
					<div class="col-md-12 mg-t-5 mg-md-t-0 ">
						<div class="az-content-body ">
							...
							<div class="az-mail-header">
								...
								<div>
								 <a href="admin/<?=$link_type?>/add" class="button_class btn btn-indigo ">Add Page</a>
								</div>
							  </div>
							  
			<ul class=" alternating_full_width_row"  table_name="tbl_static_pages" >
			...
			?>
					...
					<li   id="menu_<?=$row->id?>" class="full_width_row_<?=$row->id?> az-contact-info-header">
						...
						<nav class="nav">
							 <a href="<?=base_url().$row->slug;?>" class="badge badge-primary" target="_blank">Preview</a>
							  <a href="admin/<?=$link_type;?>/edit/<?=$row->id;?>" class="badge badge-primary">Edit</a>
							  
									<a class="badge badge-primary ajax_record_delete" data-toggle="modal" data-target="#popupDeleteRecord" item_id="<?=$row->id;?>"   table_name="tbl_static_pages" item_title="<?=$row->title;?>" section_type="full_width">Delete</a>
									
									<a href="javascript:void(0)" id="unpub_<?=$row->id; ?>" class="ajax_record_publish"  table_name="tbl_static_pages"  is_new="0">
								<div class="az-toggle az-toggle-success alternate_full_width_toogle toogle_btn <?php echo ($row->is_display == 1) ? 'on' : '';?>" publish_type="<?php echo ($row->is_display == 1) ? 0 : 1;?>"><span></span>
								<input type="hidden" name="publish_type" value="<?php echo ($row->is_display == 1) ? 0 : 1;?>" class="publish_type" />
								</div></a>
						</nav>
