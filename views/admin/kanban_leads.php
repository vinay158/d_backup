...
<?php } ?>
      ...
      <div class="board-column-content-wrapper board-column-content-wrapper_<?php echo $status->id; ?>">
		...
		<?php }?>
								...
								<div class="icon">
									...
									<?php }else{ ?>
										<span class=""  data-toggle="tooltip-primary" data-placement="top" title="" data-original-title="Mobile: <?php echo $lead->phone; ?>"><i class="fas fa-mobile-alt"></i></span> &nbsp;
									<?php } ?>
									<?php } ?>
  ...
  <?php } ?>  
	...
	<div id="popup" class="modal kanban kanban_lead_info_popup" data-backdrop="static">
		...
		$.ajax({
				...
				success:function(data){
					...
					if(data.response == 1){
						...
						//$('#responsePopup').find('.action_response_msg').html('Successfully Transferred Lead  '+data.old_kanban_status_name +' to '+data.kanban_status_name);
						/*$('#popup').modal('hide');
						
						$('#responsePopup').find('.action_response_msg').html('Successfully Transferred Lead to '+data.kanban_status_name);
						...
						*/
						$('.update_kanban_status_reponse').html('');
						$('.update_kanban_status_reponse').removeClass('won');
						$('.update_kanban_status_reponse').removeClass('lost');
						$('.update_kanban_status_reponse_code').val(1);
						if(status_type == "won"){
							$('.update_kanban_status_reponse').html('Great Job! Your lead has booked an intro!');
							$('.update_kanban_status_reponse').addClass('won');
						}else if(status_type == "lost"){
							$('.update_kanban_status_reponse').html('Lead move into archive');
							$('.update_kanban_status_reponse').addClass('lost');
						}
