...
<div class="program_full_detail new_lisiting_block default_template" id="AlternatingFullWidth">
				...
				<span style="float:right"><a  class="duplicate_mail_flow_btn ajax_record_duplicate" data-toggle="modal" data-target="#popupDuplicateItem" item_id="<?=$sparkpost_flow->id;?>"   table_name="twilio_sms_flows" item_title="<?=$sparkpost_flow->title;?>" section_type="full_width" form_action="admin/<?=$link_type?>/duplicate_form" redirect_path="admin/<?=$link_type?>">Duplicate SMS Flow</a>
				
				<?php if($sparkpost_flow->id != 1){ ?>
					<a id="delitem_<?=$sparkpost_flow->id?>" class="duplicate_mail_flow_btn ajax_record_delete" title='Delete <?=$sparkpost_flow->title;?>' data-toggle="modal" data-target="#popupDeleteRecord" item_id="<?=$sparkpost_flow->id;?>"   table_name="twilio_sms_flows" item_title="<?=$sparkpost_flow->title;?>" section_type="little_row"><i class="fa fa-trash" ></i> Delete</a>
				<?php } ?>
				</span>
			...
			<ul class=" alternating_full_width_row"  table_name="tbl_twilio_sms_templates" id="sms_flow_<?php echo $sparkpost_flow->id; ?>">
					...
					<li   id="menu_<?=$row->id?>" class="full_width_row_<?php echo $row->id; ?> az-contact-info-header">
						...
						<nav class="nav">
							...
							<?php if($user_level == 1) {?> 
							 ...
							 <a class="badge badge-primary ajax_record_delete" data-toggle="modal" data-target="#popupDeleteRecord" item_id="<?=$row->id;?>"   table_name="tbl_twilio_sms_templates" item_title="<?=$row->title;?>" section_type="full_width_row_<?php echo $sparkpost_flow->id; ?>">Delete</a>
							  <?php //} ?>
							<?php } ?>
...
<?php $i++; endforeach;?>	
			...
			<?php if(isset($sparkpost_templates['other_templates'][$sparkpost_flow->id]) && !empty($sparkpost_templates['other_templates'][$sparkpost_flow->id])){?>	
				<div class="az-content-body ">
					<div class="az-mail-header">
			...
			<!-- ajax_record_sortable  class for sort templates--->				  
			<ul class=" alternating_full_width_row"  table_name="tbl_twilio_sms_templates" id="email_flow_paid_trial<?php echo $sparkpost_flow->id; ?>">

			
			<?php
				$deleteBtnLimit = 1;
				$i = 1;
				foreach($sparkpost_templates['other_templates'][$sparkpost_flow->id] as $row):
