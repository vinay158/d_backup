...
<div class="program_full_detail page-section new_lisiting_block default_template" id="AlternatingFullWidth">
			...
			<ul class=" alternating_full_width_row"  table_name="tbl_form_autoresponders" >
			...
			if(!empty($details)):
			 ...
			 $form_instances = $this->query_model->getbySpecific('tbl_form_modules','customer_auto_responder_id', $row->id);
			...
			?>
					...
					<li   id="menu_<?=$row->id?>" class="full_width_row_<?=$row->id?> az-contact-info-header">
						...
						<nav class="nav">
							 ...
							 <a class="badge badge-primary ajax_record_delete" data-toggle="modal" data-target="#popupDeleteRecord" item_id="<?=$row->id;?>"   table_name="tbl_form_autoresponders" item_title="<?=$row->title;?>" section_type="full_width">Delete</a>
							<?php } ?>
