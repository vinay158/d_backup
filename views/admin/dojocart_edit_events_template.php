    ...
    <div class="az-content az-content-app az-content-contacts pd-b-0">
      <div class="container">
        ...
        <div class="az-content-body az-content-body-contacts">
				...
				<div class="az-mail-header">
            <div>
              <h4 class="az-content-title mg-b-5">Add DojoCart:  <?php echo $deatail->template; ?> Template</h4>
            </div>
...
$(window).load(function(){
	...
	if ($('.show_date_checkbox').is(':checked')) {
		$('.show_date_box').hide();
		$('.date_custom_text').show();
	}else{
		$('.show_date_box').show();
		$('.date_custom_text').hide();
	}

	if ($('.show_time_checkbox').is(':checked')) {
		$('.show_time_box').hide();
		$('.time_custom_text').show();
	}else{
		$('.show_time_box').show();
		$('.time_custom_text').hide();
	}
...
<div class="form-light-holder" style="">
	<h1> Additional Offer Description</h1>
...
<div class="form-light-holder">
		<label class="ckbox"><input type="checkbox" name="show_date" value="1" class="show_date_checkbox" <?php echo ($deatail->show_date == 1) ? 'checked=checked' : '' ?>><span>Show Event Date?</span></label>
</div>
<div class="show_date_box">
<div class="form-light-holder">
	<h1>Event Date</h1>
	<input type="text" value="<?php echo !empty($deatail) ? $deatail->date : date('Y-m-d') ?>" name="date" id="start_date" class="field" placeholder="mm/dd/yyyy" maxlength="10"/>
	
</div>

</div>
<div class="form-light-holder date_custom_text" style="display:none">
<h1>Custom Text</h1>
<textarea name="date_custom_text" id="frm-text"><?php echo !empty($deatail) ? $deatail->date_custom_text : '' ?></textarea>
</div>



<div class="form-light-holder">
		<label class="ckbox"><input type="checkbox" name="show_time" value="1" class="show_time_checkbox"<?php echo ($deatail->show_time == 1) ? 'checked=checked' : '' ?>><span>Show Event Time?</span></label>
</div>
<div class="show_time_box">
<div class="form-light-holder">
	<div class="adsUrl">
	<h1>Event Start Time</h1>
	<input type="text" name="start_time" value="<?php echo !empty($deatail) ? $deatail->start_time : '' ?>" class="start_timepicker" value=""/>
	</div>
	<div class="linkTarget">
	<h1>Event End Time</h1>
	<input type="text" name="end_time" value="<?php echo !empty($deatail) ? $deatail->end_time : '' ?>" class="end_timepicker"/>
	</div>
</div>

</div>
<div class="form-light-holder time_custom_text" style="display:none">
<h1>Custom Text</h1>
<textarea name="time_custom_text" id="frm-text"><?php echo !empty($deatail) ? $deatail->time_custom_text : '' ?></textarea>
</div>
...
<div class="adsUrl form-group">
<h1 style="padding-bottom: 5px;">Upload Event Image </h1>
...
<div class="form-light-holder">
	<h1>Is admission for this event paid or free?</h1>
	<select name="payment_type" class="field payment_type">
	<option value="free"<?php if($deatail->payment_type == "free"){ echo "selected";} ?>>Free Event </option>
	<option value="paid"<?php if($deatail->payment_type == "paid"){ echo "selected";} ?>>Paid Event </option>
	</select>	
</div>
...
<div class="form-light-holder salestax_required">
		...
		<h1 class="inline">Admission fee per person (<?php echo $this->query_model->getSiteCurrencyTypeForAdmin() ?>)</h1>
		...
		<h1 class="inline">Add Option for Multiple Attendees?</h1>
...
<div class="form-light-holder">
	...
	<input type="text" value="<?php echo $this->query_model->getStrReplaceAdmin($upsell_opt->up_title)?>" name="data[<?= $a ?>][up_title]" id="up_title" class="field full_width_input" placeholder="" style="width:40%"/>
</div>	
<div class="form-light-holder manage-pricerow">
...
<div class="form-light-holder">
	<label class="ckbox"><input type="checkbox" name="data[<?= $a ?>][is_qty_apply]" value="1" <?php echo ($upsell_opt->is_qty_apply == 1) ? 'checked=checked' : '';?>>  <span>Show Quantity Option For Upsell? </span></label>
</div>
<div class="form-light-holder">
...
<div class="form-light-holder">
	<label class="ckbox"><input type="checkbox" name="data[1][is_qty_apply]" value="1" >  <span>Show Quantity Option For Upsell? </span></label>
</div>		
<div class="form-light-holder">
		...
		<div class="adsUrl">
			...
			<div class="linkTarget">
				...
				<div class="amount_box<?php echo $i; ?>">
					...
					<input type="text" value="<?php echo !empty($coupon) ? $coupon->coupon_discount_amount : ''; ?>" name="Coupons[<?php echo $i; ?>][coupon_discount_amount]" id="" number="<?php echo $i; ?> coupon_required_n" class="field coupon_amount discount_amount<?php echo $i; ?>" placeholder="Value in Amount (<?php echo $this->query_model->getSiteCurrencyTypeForAdmin() ?>)"/>
					<div class="ErrorMessage ErrorMessage_price<?php echo $i; ?> coupon_required_n" style="color:#ff0000"></div>
				</div>
				
				<div class="percent_box<?php echo $i; ?>">
					<h1>#<?php echo $i; ?> Value in Percent (%)</h1>
					<input type="text" value="<?php echo !empty($coupon) ? $coupon->coupon_discount_percent : ''; ?>" name="Coupons[<?php echo $i; ?>][coupon_discount_percent]" id=""  number="<?php echo $i; ?>"  class="field coupon_percent discount_percent<?php echo $i; ?>" placeholder="Value in Percent (%)"  />
...
// Coupon code
$(".custom_required .custom_checkbox").click(function(){
	var connect_box = $(this).attr("connect_box");
	
		if($(this).hasClass("check-on")){
			$(this).removeClass("check-on");
			$(this).addClass("check-off");
			$(this).parents(".form-light-holder").children(".custom_hidden_cb").val("0");
			$("."+connect_box).show();
		}
		else
		{
			$(this).removeClass("check-off");
			$(this).addClass("check-on");
			$(this).parents(".form-light-holder").children(".custom_hidden_cb").val("1");
			$("."+connect_box).hide();
		}
...
$('.show_date_checkbox').click(function(){
	if ($(this).is(':checked')) {
		$('.show_date_box').hide();
		$('.date_custom_text').show();
	}else{
...
$('.show_time_checkbox').click(function(){
	if ($(this).is(':checked')) {
		$('.show_time_box').hide();
		$('.time_custom_text').show();
	}else{
...
<?php
	$start_time = str_replace(" : ",":",$deatail->start_time) ;
	$start_time = !empty($start_time) ? date("H:i", strtotime($start_time)) : date('h').':'.date('i');
	
	$end_time = str_replace(" : ",":",$deatail->end_time) ;
	$end_time = !empty($end_time) ? date("H:i", strtotime($end_time)) : date('h').':'.date('i');
	//echo $start_time; die;
?>

<link rel="stylesheet" href="<?php echo base_url() ?>/lightbox/date_time_picker/wickedpicker.min.css">
<script type="text/javascript" src="<?php echo base_url() ?>/lightbox/date_time_picker/wickedpicker.min.js"></script>
 <script>
$(function() {
   var options1 = { now: "<?php echo $start_time; ?>", //hh:mm 24 hour format only, defaults to current time
	...
	$('.start_timepicker').wickedpicker(options1);
  });
  
  $(function() {
   var options2 = { now: "<?php echo $end_time; ?>", //hh:mm 24 hour format only, defaults to current time
	twentyFour: false, //Display 24 hour format, defaults to false
	upArrow: 'wickedpicker__controls__control-up', //The up arrow class selector to use, for custom CSS
	downArrow: 'wickedpicker__controls__control-down', //The down arrow class selector to use, for custom CSS
	close: 'wickedpicker__close', //The close class selector to use, for custom CSS
	...
	$('.end_timepicker').wickedpicker(options2);
  });
