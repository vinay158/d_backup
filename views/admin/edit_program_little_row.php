...
<script>
	...
	CKEDITOR.replace(  'ckeditor_title', 
									{ customConfig : 'config.js' }
							);
...
$("#delete_img").click(function(){
		...
		$.ajax({ 					
		type: 'POST',						
		url: 'admin/programs/deleteProgramLittleRowImage',						
		data: { id : id}					
		}).done(function(msg){ 
		if(eval(msg) == 1){
...
<div class="form-light-holder">
	...
	<textarea type="text" name="title" id="ckeditor_title" class="field ckeditor full_width_input" placeholder=""  style=""/><?php echo !empty($pagedetails) ? $pagedetails[0]->title : '';?></textarea>
...
<div class="form-light-holder" style="overflow:auto;">
	...
	<div><img id='img' src="<?php echo base_url().'upload/programs/'.$pagedetails[0]->photo;?>" style="width: 100px; clear:both;" /></div>
	<input type="hidden" name="last-photo" value="<?=$pagedetails[0]->photo;?>" />
	<?php endif;?>
	<input type="file" name="userfile" id="photo"  accept="image/*"  />
	<?php if(!empty($pagedetails[0]->photo)){ 
			echo "<a href='javascript:void(0);' class='delete_image_btn_new'  id='delete_img' number='".$pagedetails[0]->id."'>Delete image</a>";
			}
	?>	
...
<div class="form-light-holder">
	...
	<input type="text" name="img_top_spacing" id="img_top_spacing" class="field  img_top_spacing" placeholder=""  style="width: 7%" value="<?php echo $this->query_model->getStrReplaceAdmin($pagedetails[0]->img_top_spacing)?>"/> <span style="font-size:15px"><strong>px</strong></span><br/>
