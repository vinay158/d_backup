...
<div class="az-content-body-left advanced_page custom_full_page onlinespecial_promo_codes" >
						...
						<div class="az-content-body ">
							...
							<div class="az-mail-header">
								<div>
								  ...
								  <p>You have <span class="total_alternating_full_width_row"><?php echo !empty($promo_codes) ? count($promo_codes) : 0; ?></span> Entries</p>
								</div>
								<div>
								 <a href="admin/<?=$link_type?>/add_promocodes" class="button_class btn btn-indigo ">Add New</a>
								</div>
							  </div>
							  
			<ul class="alternating_full_width_row"  table_name="tbl_onlinespecial_promo_codes" >
					...
					<li   id="menu_<?=$row->id?>" class="full_width_row_<?=$row->id?> az-contact-info-header">
						<div class="manager-item media">
							<div style="float:left;">
								...
								<h4 class="full_width_row_heading_<?=$row->id?>"><a href="javascript:void(0)" ><?=$row->title;?> </a>
						...
						<nav class="nav">
							  ...
							  <a href="admin/<?=$link_type?>/edit_promocodes/<?=$row->id;?>" class="badge badge-primary">Edit</a>
							  
									<a class="badge badge-primary ajax_record_delete" data-toggle="modal" data-target="#popupDeleteRecord" item_id="<?=$row->id;?>"   table_name="tbl_onlinespecial_promo_codes" item_title="<?=$row->title;?>" section_type="full_width">Delete</a>
							<?php if($row->expiry_date < date('Y-m-d')){ 
										// write some code here... 
									}else{ 
							?>		
									<a href="javascript:void(0)" id="unpub_<?=$row->id; ?>" class="ajax_record_publish"  table_name="tbl_onlinespecial_promo_codes"  is_new="0">
