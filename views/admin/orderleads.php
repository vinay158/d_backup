				...
				<div class="panel-body-holder">
					<div class="manager-items custom">
						<div class="border floatNone ">
							<h1><?=$title?> <a href="<?=base_url();?>admin/leads/exportorderleads/?location=<?php echo (isset($_GET['location'])) ? $_GET['location'] : ''; ?>" class="button_class">Export List</a> 
                            ...
                            <table cellpadding="2" cellspacing="2" width="100%" id="myTable" class="tablesorter ">
                                <thead>
                                   ...
                                   <!-- <th align="left"> &nbsp; </th> -->
									...
									<th align="left">Upsell</th>
									<th align="left">Trial Type</th>
                                ...
                                <tr>
                                   <!-- <td><input type="checkbox" class="checkbox_list" name="lead_ids[<?php echo $order->id ?>]" value="<?php echo $order->id ?>"></td> -->
									...
									<td><?php echo !empty($order->last_order_id) ? 'Yes' : 'No'; ?></td>
									 ...
									 || 
										...
										<div class="view_lead_info" id="lead_<?php echo $order->id ?>" title="<?= $title ?>" style="display:none;">
										  <table  cellpadding="2" cellspacing="2" width="100%">
											...
											<tr>
												<td>Child's Name : <?=ucfirst($order->child_name)?></td>
												<td>Child's Age : <?=$order->child_age?></a></td>
											</tr>
											<?php } ?>
											...
											<tr> 
												<td>Trial Name: <?php echo !empty($offerDetail) ? $offerDetail[0]->offer_title : ''; ?></td>
											
												<td>
												<?php if(empty($order->last_order_id)){ ?>
												Trial Type : <?php if(!empty($order->offer_type)){ echo $order->offer_type; }  ?> 
											...
											<tr>
											<td>Upsell : <?php echo !empty($order->last_order_id) ? 'Yes' : 'No'; ?></td>
											
											<td>Upsell Name: <?php echo !empty($order->upsells_title ) ? str_replace( 'Upsell :-','',$order->upsells_title)  : ''; ?></td>
											
											</tr>
											<tr>
												<td>Coupon Code: <?php echo !empty($order->coupon_code) ?$order->coupon_code : ''; ?></td>
												<td>Coupon Discount : <?php echo !empty($order->offer_type) ? $site_currency_type.$order->coupon_discount : '';  ?></td>
												
											</tr>
											
											<tr>
												<td>Status : <?php echo $order->trans_status; ?></td>
												<td>Amount : <?php if(!empty($order->trial_id)){ if(!empty($order->amount)) { echo $site_currency_type.$order->amount; } else { echo ''; } } ?></td>
