...
<div class="az-content-body-left advanced_page custom_full_page twillio_msg_templates" >
	<div class="az-content-header d-block d-md-flex">
		<div class="az-dashboard-one-title">
			<div>
			  <h2 class="az-dashboard-title">Twilio API Messages</h2>
			</div>
...
<?php if(!empty($pagedetails) && $pagedetails[0]->type == 1){ ?>
...
<div class="mb-3 main-content-label page_main_heading">Twilio Message Settings</div>
...
<div class="row row-xs align-items-center">
	...
	<div class="row mg-t-10">
		<div class="col-lg-4">
			<div class="form2checkbox">
			<a id="published" class="checkbox2 <?php echo (!empty($pagedetails) && $pagedetails[0]->send_user_msg == 1) ? 'check-on' : 'check-off'; ?>"></a>

			<h1 class="inline">Send Message To Customer </h1>

			<input type="hidden" value="<?php echo (!empty($pagedetails) && $pagedetails[0]->send_user_msg == 1) ? 1 : 0; ?>" name="send_user_msg" class="hidden_cb2" />
			
			</div>
		</div><!-- col-3 -->
		<div class="col-lg-4 mg-t-20 mg-lg-t-0">
			<div class="form1checkbox">
	
			<a id="published" class="checkbox1 <?php echo (!empty($pagedetails) && $pagedetails[0]->send_admin_msg == 1) ? 'check-on' : 'check-off'; ?>"></a>

			<h1 class="inline">Send Message To Admin </h1>

			<input type="hidden" value="<?php echo (!empty($pagedetails) && $pagedetails[0]->send_admin_msg == 1) ? 1 : 0; ?>" name="send_admin_msg" class="hidden_cb1" />
		</div>
		</div><!-- col-3 -->
		<div class="col-lg-4 setting_btn_box">
...
<div class="mb-3 main-content-label page_main_heading">Twilio Message Templates</div>
...
<?php foreach($formTypes as $key => $val){ ?>

<div class="form-light-holder d-md-flex  dual_input">

	<div class="adsUrl form-group" >
		<h1 class="label-text"><?php echo $val; ?></h1>
		<h1>Client Message Template</h1>
		<textarea name="twillio_msg_template[<?php echo $key; ?>][client_msg]" rows="5"><?php echo isset($twillioMsgTemplates[$key]) ? $twillioMsgTemplates[$key]->client_msg : ''; ?></textarea>
	</div>
	
	<?php if(!empty($pagedetails) && $pagedetails[0]->send_admin_msg == 1) { ?>
	<div class="linkTarget form-group second_textarea_box">
		<h1 class="label-text"> &nbsp; </h1>
		<h1>Admin Message Template</h1>
	<textarea name="twillio_msg_template[<?php echo $key; ?>][admin_msg]"  rows="5"><?php echo isset($twillioMsgTemplates[$key]) ? $twillioMsgTemplates[$key]->admin_msg : ''; ?></textarea>
	</div>	
	<?php } ?>
