...
<div class="az-content-body-left advanced_page custom_full_page rank_traker_api_page" >
	<div class="az-content-header d-block d-md-flex">
		<div class="az-dashboard-one-title">
			<div>
			  <h2 class="az-dashboard-title"><?php echo $_SERVER['HTTP_HOST']; ?> SEARCH ENGINE RANKINGS </h2>
			</div>
	...
	$(document).ready(function(){
		$('.url_types').change(function(){
			...
			$.ajax({
				...
				url : '<?php echo base_url("admin/search_engine_rankings_report/ajaxGetRankTrackrKeywords"); ?>',
					type : 'POST',
					dataType :'json',
...
<div class="panel-body ordersListTable rank_traker_response">
			...
			<div class="card card-dashboard-twentyfive keyword-bar card-body box_1 ">				<div class="row keyword_box_data1">              <div class="col-12 custom-keyword-box">					<iframe src="https://player.vimeo.com/video/283955989?title=0&byline=0&portrait=0" width="100%" height="600" frameborder="0" webkitAllowFullScreen mozallowfullscreen allowFullScreen></iframe>              </div>            </div>			</div>
            <div class="card card-dashboard-twentyfive keyword-bar card-body box_1"  style="display:none">
			...
			<div class="card card-dashboard-twentyfive keyword-bar card-body box_2"  style="display:none">
			<div class="row keyword_box_data1">
              <div class="col-6 col-sm-3 custom-keyword-box first"  style="display:none">
                <div class="chart-wrapper">
		...
		<?php if(isset($api_reponse_data['keywords_list']) && !empty($api_reponse_data['keywords_list']) ){ ?>
			<div class="col-sm-12 col-xl-12 col-lg-12 nopadding">
				...
				<th class="wd-10p">Search Engine <span data-container="body" data-popover-color="head-primary" data-placement="top" title="Search Engine" data-content="Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat."><i class="fa fa-question-circle"></i></span>				<select class="search_engines" style="width:100px">					<?php 						if(isset($search_engines) && !empty($search_engines)){							foreach($search_engines as $search_engine){  														$selected_engine = "google";							$engine = str_replace(' ','',$search_engine);					?>						<option value="<?php echo $engine; ?>" <?php echo ($selected_engine == $engine) ? 'selected=selected' : ''; ?>><?php echo ucfirst($search_engine); ?></option>					<?php } } ?>				</select>				</th>
				...
				<select class="competitors" style="width:100px">
					<?php 
				...
				<th class="wd-5p">Action<span data-container="body" data-popover-color="head-primary" data-placement="top" title="Action" data-content="Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat."><i class="fa fa-question-circle"></i></span></th>
			...
			<?php 
				$n = 1;
				foreach($api_reponse_data['keywords_list'] as $k => $keyword){ 					$search_engine = str_replace(' ','',$keyword->engine);
			?> 
				<tr  class="all_search_engine search_engine_<?php echo $search_engine; ?>">
				<td ><?=$n?>.</td>
				<td>
				<?php 
					$search_engine = '';
				...
				<a href="<?php echo base_url().'admin/search_engine_rankings_report/keyword/'.$keyword->id; ?>"><i class="fas fa-chart-bar"></i></a></td>
				</tr>
			<?php $n++; }?>	
		...
		<?php } ?>	
		<div class="col-sm-12 col-xl-12 col-lg-12 nopadding">				<div class="mb-3 main-content-label" >Add Competitor</div>				<div class=" card-dashboard-twentyfive keyword-bar backlinks-link card-body">					<form id="" action="<?php echo base_url().'admin/search_engine_rankings_report/add_competitor'; ?>" method="post">						<div class="form-new-holder">							<div class="adsUrl">								<h1>Competitor Site URL</h1>								<input type="text" value="" name="competitor_site_url" id="competitor_site_url" class="field" placeholder="Enter Competitor Site URL">								<br/><em>eg: http://example.com</em>								<input type="hidden" name="api_url_id" value="<?php echo $api_url_id; ?>">								<input type="hidden" name="action" value="add_competitor">							</div>														<div class="linkTarget">								<h1>&nbsp; </h1>								<input type="submit" name="update" value="Save" class="btn-save" style="margin-top: 15px !important;">							</div>													</div>						</form>				</div>			</div></div>
		...
		<?php if(isset($api_reponse_data['competitors_list']) && !empty($api_reponse_data['competitors_list']) ){ ?>
			...
			<div class="col-sm-12 col-xl-12 col-lg-12 nopadding">
				...
				<!--<th class="wd-30p" style="width:100px">Action <span data-container="body" data-popover-color="head-primary" data-placement="top" title="Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. " data-content="Action"><i class="fa fa-question-circle"></i></span></th>-->
				</tr>
				</thead>
			...
			?> 
				...
				<td ><?php echo $competitor->url; ?></td>								<!--<td ><a href="<?php echo base_url().'admin/search_engine_rankings_report/delete_competitor?action=delete&api_url_id='.$api_url_id.'&competitor_id='.$competitor->id; ?>" class="delete_btn"  onclick="return confirm('Are you sure you want to delete this competitor?')"><i class="fa fa-trash"></i></a></td>-->
				</tr>
			<?php $n++; }?>	
...
$(window).load(function(){
   ...
   var competitor_id = $('.competitors').val();
	 ...
	 $('.competitor_rank_'+competitor_id).show();
  	 var search_engine_val = $('.search_engines').val();		 $('.all_search_engine').hide();	 $('.search_engine_'+search_engine_val).show();
})
...
if ($('#chart-<?php echo $key; ?>-container').length){
  ...
  $('.competitors').change(function(){
    ...
    $('.search_engines').change(function(){	 var search_engine = $(this).val();	 $('.all_search_engine').hide();	 $('.search_engine_'+search_engine).show();  })  
});
</script>
<?php 
