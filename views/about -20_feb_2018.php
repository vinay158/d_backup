...
<link rel="stylesheet" type="text/css" href="lightbox/jquery.fancybox.css?v=2.1.5" media="screen" />
            ...
            <div class="row">
               <div class="col-md-12 text-center">
				  ...
				  <?php if(!empty($aboutEmailOption) && !empty($aboutEmailOption[0]->opt1_text)) {
					...
					echo 'Enter your Email to View Current Web-Specials & Pricing';;  
				  }  ?>
				  </h2>
               </div>
            </div>
	   ...
	   <span></span>
		...
		$('.contactFormSubmit_8').click(function(){
					...
					if(email.length == 0 || emailfilter.test($("#form_email_8").val()) == false){
						var err = 1;
						$('#form_email_8').after('<div class="reds email_error8">Enter a valid email address</div>');
					}
	...
	<form action="#"  method="post" class="get_started_form mini_form" id="form_8">
                  <div class="search-box">
                     <input type="text" class="form-control" id="form_email_8" name="form_email_2" placeholder="Enter your email address">
                     ...
                     <button class="btn-red contactFormSubmit_8" name="submitEmail" value="submitEmail" type="submit" >GO</button>
                  </div>
			  </form>
				   </div>
				   </div>
         ...
         <div class="container">
            <div class="row">
               <div class="col-md-12 col-sm-12 text-center">
                  ...
                  <h2><?php if(!empty($aboutHeader)){ $this->query_model->getStrReplace($aboutHeader[0]->title); }?>
                  </h2>
	...
	<form action="#"  method="post" class="get_started_form mini_form" id="form_6">
                  <div class="search-box">
					 ...
					 <input type="hidden" name="school_interest" value="<?php echo !empty($page_location_id) ? $page_location_id : ''; ?>" />
					 <input type="hidden" name="miniform" value="true" />
                     <button class="btn-red contactFormSubmit_6" name="submitEmail" value="submitEmail" type="submit" >GO</button>
                  </div>
			  </form>
				   </div>
				   </div>
...
<div class="trial-form about-trial-form about-alt-bg">
  <div class="container">
    ...
    <div class="row">
	  ...
	  <?php 
		...
		if($multiLocation[0]->field_value == 0){
			$videoSection = (!empty($aboutContent) && $aboutContent['video_section'] == 1) ? 1 : 0 ;
		}else{
         ...
         <div class="container">
            <div class="row">
			 ...
			 <?php if($multiLocation[0]->field_value == 0){ ?>
			   <?php if($aboutContent['image_video'] == 'video'){  ?>
				 <?php if($aboutContent['video_type'] == 'youtube_video'){ ?>
						<?php if(!empty($aboutContent['youtube_video'])){ ?>
								  <div class="video-inner">
									 <iframe  height="390" src="<?php echo $this->query_model->changeVideoPathHttp($aboutContent['youtube_video']); ?>?rel=0&amp;controls=0&amp;showinfo=0&amp;autoplay=0" frameborder="0" allowfullscreen ></iframe>
									 <span class="video-overlay">
										<div class=""></div>
									 </span>
								  </div>
								   <?php } } else{ ?>
							  <?php if(!empty($aboutContent['vimeo_video'])){ ?>
							  <div class="video-inner">
									 <iframe  height="390" src="<?php echo $this->query_model->changeVideoPathHttp($aboutContent['vimeo_video']); ?>?rel=0&amp;controls=0&amp;showinfo=0&amp;autoplay=0" frameborder="0" allowfullscreen ></iframe>
									 <span class="video-overlay">
										<div class=""></div>
									 </span>
								  </div>
								<?php } } ?> 
							<?php } else { ?>
							<?php if(!empty($aboutContent['photo'])){ ?>
							<img class="img-responsive center-block aboutPageImg" src="<?php echo base_url().'upload/about_us/'.$aboutContent['photo']; ?>" alt="<?= $this->query_model->getDescReplace($aboutContent['image_alt']); ?>" />
                  ...
                  <div class="video-block text-center">
                     <h2><?= $this->query_model->getStrReplace($aboutContent['title']); ?></h2>
                     <p><?= $this->query_model->getDescReplace($aboutContent['sub_title']); ?></p>
                  </div>
			   ...
			   <?php }else{ ?>
				  ...
				  <?php if(!empty($aboutContent[0]->vimeo_video)){ ?>
				   <div class="video-inner">
                     <iframe  height="390" src="<?php echo $this->query_model->changeVideoPathHttp($aboutContent[0]->vimeo_video); ?>?rel=0&amp;controls=0&amp;showinfo=0&amp;autoplay=0" frameborder="0" allowfullscreen ></iframe>
                     <span class="video-overlay">
                        <div class=""></div>
                     </span>
                  </div>
     ...
     <section class="web-special">
	  ...
	  <section class="trial-form trial-form-about-footer">
         <div class="container">
            <div class="row">
               <div class="col-md-12 text-center">
               	<h2>imagine yourself or your child as a strong, confident leader.</h2>
                  <p>This is your chance to try our martial arts program at a reduced price with no commitments!</p>
                  <h3 class="head-one">1: choose program of interest</h3>
                  <!-- <h2> 
				  <?php if(!empty($aboutEmailOption) && !empty($aboutEmailOption[0]->opt_2_title)) {
						$this->query_model->getDescReplace($aboutEmailOption[0]->opt_2_title); 
				  }else{
					echo 'Limited Trial Offers Available';
				  }  ?></h2>
                  <p> 
				  <?php if(!empty($aboutEmailOption) && !empty($aboutEmailOption[0]->opt_2_text)) {
						$this->query_model->getDescReplace($aboutEmailOption[0]->opt_2_text); 
				  }else{
					echo 'If you want to avoid paying regular pricing or missing out, <br>this is your chance to try our martial arts program for a ridiculously low price<br><strong>TAKE ACTION NOW!</strong>';
				  }  ?>
				  </p> -->
		...
		$('.trialButton').click(function(){
			...
			?>	  
				...
				<div class="clearfix"></div>
                  ...
                  <h3>2: Opt–in to redeem trial offer</h3>
	...
	<form action="#"  method="post" class="get_started_form mini_form mini_trial_offer_form" id="form_7">
                  <div class="search-box">
...
?>
	  ...
	  <script>
               ...
               <li class="<?php echo $ShowMoreIntImg; ?> instragramImages"  number="<?php echo $i; ?>">
				 ...
				 <div class="insta-caption" >
					<div class="review-detail manage_lightboxthumb">
					   <a href="<?php echo $main_img; ?>" class="fancybox"  data-fancybox-group="gallery" title="Likes: <?php echo $vm['likes']['count']; ?> &nbsp; Comments: <?php echo $vm['comments']['count']; ?>"> 
					  <p><span>
					 
					  <i class="fa fa-heart"></i> Likes:<?php echo $vm['likes']['count']; ?></span> <span><i class="fa fa-comment"></i> Comments: <?php echo $vm['comments']['count']; ?></span> </p>
					</a>
					</div>
				  </div> 
               </li>
		 ...
		 <?php if(count($ins_media['data']) > 5){ ?>
        	<a href="javascript:void(0);" class="load-more showMoreInstragram">Load More</a> 
		<?php } ?>
            <a href="#" class="follow-btn"><i class="fa fa-instagram"></i> Follow </a>
         </div>
      </section>
<?php } ?>
      ...
      <div class="row">
        ...
        <div class="row">
		  ...
		  <?php if($multiLocation[0]->field_value == 1 ){ ?>
			...
			<?php } else{ ?>
		  <a class="btn btn-theme" href="<?php echo  $this->query_model->getFindUs($contactDetail->address,$contactDetail->suite,$contactDetail->city,$contactDetail->state,$contactDetail->zip); ?>"  target="_blank"> <i class="fa fa-map-marker"></i> Find Us </a>
		  <?php } ?>
		   </div>
        </div>
      </div>
