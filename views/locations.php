...
function initialize() {
	...
	map.setCenter(new google.maps.LatLng(38.5556, -121.4689));
...
google.maps.event.addDomListener(window, 'load', initialize);
	...
	$("#submit").click(function(e){
	  ...
	  $("#form_school_2").change(function(e){
		  e.preventDefault();
		  var base_url = '<?=base_url()?>';
		  var location_name = $(this).val();
		  
		 // alert('location_id: '+location_name+' base url '+base_url);		
		  
		  $.ajax({
			  url: base_url+'locations/getlocationdatabyname/'+location_name,			  
			  dataType: 'json',
			  success: function(data) {
				// alert(data.name+'\n'+data.content);
...
function sticky_relocate() {
    ...
    if (window_top > div_top) {
        $('#contact-area').addClass('stick');
    } else {
        $('#contact-area').removeClass('stick');
    }
}
...
if($msg == 'Thank You'){
?>
		<h1 style="text-align:center; margin-top: 10px">Thank You!</h1>
		<p style="text-align:center;margin-top:5px; color: #000">Your message was sent!</p>
<?php	
}elseif($msg == 'failed'){
	?>
    	<h1 style="text-align:center; margin-top: 10px">Sending failed</h1>
		<p style="text-align:center;margin-top:5px; ; color: #000">Please resend your enquiry!</p>
	...
	<div class="main_content light" id="top">
        ...
        <div id="google-map">
			<p id="map-search">
				...
				<!--- <input type="button" name="submit" id="findAddress" value="Submit" class="zip-search-go" onClick="codeAddress(document.getElementById('search').value)"> -->
                <input type="button" name="submit" id="submit" value="Submit" class="zip-search-go">
			</p>
               ...
               foreach($contactlocations as $location){
                ...
                ?>
                    <div class="list-text-item">
                        ...
                        <ul>
                        	<?php
								if(!empty($facility_slug) && $location->published == 1){
                ...
                <form method="post" class="contact_form content_contact_form" action="locations/send">
                    ...
                    <ul class="form_fields">
                        ...
                        <?php
							...
							$programs = $prg_data->result();
						?>
                        
                        <li class="clearfix">
                        
                            <label class="form_program_2">Program Of Interest</label>        
                                    
                            <select name="program" id="form_program_2">
